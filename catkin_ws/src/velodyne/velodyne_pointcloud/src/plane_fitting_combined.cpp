...
float calculateDistanceToPlane(const pcl::ModelCoefficients& coefficients, const pcl::PointXYZRGB& point) {
...
// 
    ...
    // Determine gris size for firest tiling mechanism
    int rows = ceil(x_strip / strip_size) ;
    int cols =ceil((2 * y_strip) / strip_size);

    // determine grid size for second tiling mechanism 
    float strip_size1= 0.50;
    int rows1=ceil(x_strip/ strip_size1);
    int cols1=ceil((2*y_strip)/ strip_size1);
    //  ------


    // Create 2D array for grid 1
    std::vector<std::vector<pcl::PointIndices>> inliers_array(rows, std::vector<pcl::PointIndices>(cols));
//    creating 2d array for grid2
    std::vector<std::vector<pcl::PointIndices>> inliers_array1(rows1, std::vector<pcl::PointIndices>(cols1));
...
for (int i = 0; i < filtered_cloud->size(); ++i) {
    ...
    int row_index1=static_cast<int>((point.x - x_strip_initial)/strip_size1);
    int col_index1 = static_cast<int>((point.y + y_strip) / strip_size1);
    inliers_array[row_index][col_index].indices.push_back(i); // Store the index of the point
    
     inliers_array1[row_index1][col_index1].indices.push_back(i);  // store the point index in second grid (0.5)
}
...
// 
    ...
    //  for second grid 
    std::vector<std::vector<pcl::ModelCoefficients>> coefficients_array1(rows1, std::vector<pcl::ModelCoefficients>(cols1));
...
for (int i = 0; i < rows; ++i) {
    ...
    //    for second grid plane fitting in each tile
    for (int i = 0; i < rows1; ++i) {
        for (int j = 0; j < cols1; ++j) {
            // Check if the number of points is sufficient for RANSAC
            if (inliers_array1[i][j].indices.size() <= min_points_for_ransac) {
                coefficients_array1[i][j].values.assign(4, 0); // Set parameters to (0, 0, 0, 0)
                continue; // Skip RANSAC for this grid cell
            }
            ...
            pcl::ExtractIndices<pcl::PointXYZRGB> extract;
            extract.setInputCloud(filtered_cloud);
            extract.setIndices(boost::make_shared<pcl::PointIndices>(inliers_array1[i][j]));
            extract.filter(*extracted_cloud);
            // Fit plane to extracted points
            ...
            // Store model coefficients in array
            coefficients_array1[i][j] = *coefficients;
        }
    }
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        //  for grid2 printing
          for (int i = 0; i < rows1; ++i) {
            for (int j = 0; j < cols1; ++j) {
                std::cout << "Grid1 (" << i << ", " << j << ") - Indices: ";
                for (size_t k = 0; k < inliers_array1[i][j].indices.size(); ++k) {
                    std::cout << inliers_array1[i][j].indices[k] << " ";
                }
                
                std::cout<<",Count "<<inliers_array1[i][j].indices.size();
                std::cout << std::endl;
                std::cout << "Plane coefficients for grid (" << i << ", " << j << "): ";
                const pcl::ModelCoefficients& coefficients = coefficients_array1[i][j];
                std::cout << "a: " << coefficients.values[0] << ", "
                          << "b: " << coefficients.values[1] << ", "
                          << "c: " << coefficients.values[2] << ", "
                          << "d: " << coefficients.values[3] << std::endl;
...
for (size_t i = 0; i < filtered_cloud->points.size(); ++i) {
    ...
    const pcl::ModelCoefficients& coefficients1 = coefficients_array1[row_index1][col_index1];

    float distance= calculateDistanceToPlane(coefficients,point);
    float distance1= calculateDistanceToPlane(coefficients1,point);

    float avg_dist= (distance + distance1 ) / 2.0 ;
    ...
    for (int j = 0; j < sizeof(severity_thresholds) / sizeof(severity_thresholds[0]); ++j) {
        if (avg_dist* 100 >= severity_thresholds[j]) {
...
int main(int argc, char **argv)
{
  ...
  pub_ = nh_ground_removal_new.advertise<sensor_msgs::PointCloud2> ("/test_topic_plane_fitting_combined", 1);
